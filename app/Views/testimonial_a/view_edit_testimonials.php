                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-warning">
                        <div class="card-header">
                            <h2 class="card-title">Search Testimonials</h2>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <div class="card-body ">
                            <form name="quickForm" id="quickForm">
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <?php
                                            $total_count = $testimonials_count;
                                    ...
                                    <div class="col-md-12">
                                        <button class="btn btn-primary float-right" onclick="searchTestimonials(event, this)">Search</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="card-footer">
                            <p><?php echo $testimonials_count ?> - records found with search filters</p>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        ...
                                        if (!empty($testimonials))
                                        {
                                            ...
                                            foreach ($testimonials as $testimonial)
                                            {
                                                ...
                                                <tr>
                                                    <td><?php echo $sno++ ?></td>
                                                    <td><?php echo $helperModel->get_d_MM_Y_h_ia_from_date_string($testimonial['created_on']) ?></td>
                                                    <td><?php echo $testimonial['user_name'] ?></td>
                                                    <td><?php echo $testimonial['no_of_stars'] ?></td>
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-info">
                <p class="modal-title">Testimonial Info</p>
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <p class="modal-title">Update Testimonial</p>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group required d-none">
                                <label for="testimonial_id">Testimonial ID</label>
                                <input type="text" name="testimonial_id" class="form-control" placeholder="Testimonial ID">
                            </div>
                            <div class="form-group">
...
<script>
    ...
    function viewTestimonial(testimonial_id, e)
    {
        enableDisableDiv('#datatable', false);
        var submitData = {};
        submitData['testimonial_id'] = testimonial_id;
        var submittedData = {
            'submittedData': submitData
        };
        $.ajax({
            url: '<?php echo $custom_site_url . $controller; ?>/ajax_request/testimonial/info',
            datatype: 'json',
            type: 'POST',
            data: submittedData,
            ...
            success: function (response)
            {
                ...
                {
                    var receivedDataArray = receivedData.testimonials;
                    $.each(receivedDataArray, function (idx, row)
                    {
                        $('[name="view_modal"]').find('[name="name"]').text(row.testimonial_name);
    ...
    function addFollowUp(testimonial_id)
    {
        enableDisableDiv('#datatable', false);
        var submitData = {};
        submitData['testimonial_id'] = testimonial_id;
        ...
        $.ajax({
            ...
            success: function (response)
            {
                ...
                if (receivedData.success == 1)
                {
                    ...
                    $.each(receivedDataArray, function (idx, row)
                    {
                        $('[name="add_follow_up_modal"]').find('[name="name"]').text(row.testimonial_name);
                        ...
                        $('[name="add_follow_up_modal"]').find('[name="testimonial_id"]').val(row.testimonial_id);
                    });
    ...
    function updateFollowUp(ev, e)
    {
        ...
        testimonial_id = $('[name="add_follow_up_modal"]').find('[name="testimonial_id"]').val();
        ...
        $.ajax({
            url: '<?php echo $custom_site_url . $controller; ?>/view_edit_testimonials/add_follow_up',
            datatype: 'json',
            type: 'POST',
            data: submittedData,
            success: function (response)
    ...
    function viewFollowUp(testimonial_id, e)
    {
        enableDisableDiv('#datatable', false);
        var submitData = {};
        submitData['testimonial_id'] = testimonial_id;
        ...
        $.ajax({
            url: '<?php echo $custom_site_url . $controller; ?>/ajax_request/testimonial/follow_up',
            datatype: 'json',
            type: 'POST',
            data: submittedData,
            success: function (response)
    ...
    function searchTestimonials(ev, e)
    {
        ev.preventDefault();
        ...
        $.ajax({
            url: '<?php echo $custom_site_url . $controller; ?>/view_edit_testimonials/search',
            datatype: 'json',
            type: 'POST',
            data: submittedData,
            success: function (response)
